# Accuracy Visualization Script
library(readxl)
library(dplyr)
library(tidyr)
library(ggplot2)

file_path <- "data/accuracy.xlsx"
...
all_data$Response <- factor(all_data$Response,
                            levels = c("completely incorrect", "mixed", "correct"))
all_data$Model <- factor(all_data$Model, levels = sheet_names)

my_colors <- c(
  "completely incorrect" = "#FF7F0E",
  "mixed" = "#AED6F1",
  "correct" = "#1A5276"
)
...
ggsave("figures/basic_accuracy_chart.png", p, width = 8, height = 6, dpi = 300)
